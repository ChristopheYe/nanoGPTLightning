...
with open("input.txt", "r") as f:
    text = f.read()  # That's what you do for a .txt file
...
# encode the sentence in a list of character
encode = lambda s: [
    stoi[c] for c in s
...
class Block(nn.Module):
    def __init__(self, n_embd, n_head, dropout=0.2):
        ...
        self.ma = MultiHeadAttention(n_embd, n_head, head_size, dropout)
...
class GPTLanguageModel(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, vocab_size, block_size, dropout):
        super().__init__()
...
model = GPTLanguageModel(n_embd, n_head, n_layer, vocab_size, block_size, dropout)
m = model.to(device)
